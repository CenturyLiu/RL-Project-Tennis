...
BUFFER_SIZE = int(1e6)
NUM_SUB_POLICY = int(10)
BATCH_SIZE = 128
N_LEARN_UPDATES = 4     # sample N_LEARN_UPDATES batches during training
UPDATE_EVERY = 4
...
def run(max_episode, max_steps, batch_size):
    ...
    episode_rewards = []
    update_sub_policy_array = np.zeros(NUM_SUB_POLICY)
    for episode in range(max_episode):
        ...
        for step in range(max_steps):
            ...
            update_sub_policy_array[sub_policy_id] += 1
            #print(update_sub_policy_array[sub_policy_id])
            ...
            if all(dones) or step == max_steps - 1:
                ...
                maddpg_train.replay_buffer[sub_policy_id].push(states, [a.detach() for a in actions], rewards, next_states, dones)
                states = next_states 
                if len(maddpg_train.replay_buffer[sub_policy_id]) > batch_size and update_sub_policy_array[sub_policy_id] % UPDATE_EVERY == 0:
                    update_sub_policy_array[sub_policy_id] = 0
